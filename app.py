...
def get_gradcam_heatmap(pil_img, depth="Bottom"):
    ...
    # 🔥 FIX: faster + safer CAM computation
...
def process_prediction(pil_img, depth="Bottom"):
    ...
    # predict → ensure clean numpy array
    preds = model.predict(arr[np.newaxis, ...], verbose=0)
    preds = np.asarray(preds).squeeze()   # 🔥 safest + most robust fix

    # ensure shape is (4,) not (1,4) or weird nested array
    preds = preds.reshape(-1)

    idx = int(np.argmax(preds))  # safe scalar index
    ...
    return {
        ...
        "probabilities": probs,
        "heatmap_image": pil_to_base64(
            get_gradcam_heatmap(pil_img, depth)
        ),
    }
